...
\usepackage{pdfpages}                % for attaching the project outline and risk assessment PDFs in the appendix
...
\wordcount{4833}		                   % Automatic \mywordcount is broken on this machine (texcount.pl emits Perl warnings to stdout that corrupt \CatchFileEdef's capture); this number was set by running `texcount -1 -sum -merge 14354077.tex` directly. Re-run that command and update this value whenever the document changes significantly.
\maketitle
...
\begin{uomacknowledgements}
  I would like to thank my parents, Geetha Selvakumar and Selvakumar Palaniyandi, for their constant support throughout this degree. Thanks also to my younger brother, Saran Selvakumar, who has never once let me take myself too seriously, and to my friends who pushed me to keep going when this project got difficult.

  I am grateful to my supervisor, Dr.~Pawel, for their guidance throughout this project. A special thanks to Swetha Parthasarathy, a good friend, whose encouragement pushed me to go further than I thought I could.
...
%%%%%%%%%%%%%%%%%% APPENDICES %%%%%%%%%%%%%%%%%%
% Not using the uomappendix environment directly: it forces a standalone
% "Appendices" heading page via \section*{Appendices} before any content,
% which left a near-empty page above since \includepdf always starts its
% content on its own fresh page. Replicating uomappendix's bookmark/ToC
% setup without the printed heading lets the first appendix PDF page follow
% the references immediately.
\clearpage
...
\includepdf[pages=-,addtotoc={1,section,1,Project Outline,app:project-outline}]{appendix_pdfs/Project_Outline.pdf}

\includepdf[pages=-,addtotoc={1,section,1,Risk Assessment,app:risk-assessment}]{appendix_pdfs/Risk_Assessment_WFH.pdf}
