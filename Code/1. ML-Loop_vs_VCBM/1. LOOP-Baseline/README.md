...
This repository is a fork of Apple's original LOOP reference implementation (above), adapted for a
Postgraduate Dissertation project at the University of Manchester, run on the CSF3 HPC cluster. The fork
adds a credit-assignment extension, **VCBM (Visual Causal Chain Bookmarking)**, under `phi_agents/rl/vcc/`.
Original Apple-authored files are unchanged in license/attribution; new fork-local additions do not carry
the Apple copyright header. See [CLAUDE.md](CLAUDE.md) for fork-specific setup, commands, the CSF3
workflow, and VCBM architecture notes; the rest of this README documents the original upstream project.
